...
| `template_engine.py` | Extract template volumes; render compose and nginx files via Jinja2; copy `.env` as per-user file + rewrite `env_file:` refs; rewrite static `proxy_pass` → variable-based (`set $upstream_XXXX` + `proxy_pass http://$upstream_XXXX`) for per-request DNS resolution so nginx reloads cleanly even with missing upstream containers; pass `subnet` / `gateway` into compose templates (guarded by `{% if subnet %}`); always inject the v4 server scaffold (byte-identical per-service conf — no `ENABLE_ACL` branch) and write the env.d mode one-liner (`set $auth_mode acl;|basic;`); `location = /_set_token` is a plain variable proxy to the gateway exchange (no JWT in URL) |
...
| `nginx_converter.py` | Apply regex substitutions to a plain nginx conf and emit a `.j2` template; injects `auth_basic` + `auth_basic_user_file` directives before the first `proxy_pass` if none are already present; detects when a `proxy_pass` host matches a compose service name and rewrites it to `{{ container_prefix }}<name>`; injects `location = /_set_token` into every server block and normalizes legacy `return 302 $arg_redirect;` → `$scheme://$http_host$arg_redirect;`; injects the `location = /_auth_jwt` gateway subrequest and the ACL server-level directives |
...
Input: user_name, service_name, label, volumes, passwd, template paths, env_file?
  ...
  │       │       └─ v4: always injects the v4 server scaffold (byte-identical conf); the
  │       │          env.d one-liner (set $auth_mode acl;|basic;) switches the mode
...
13. **`ENABLE_ACL` switches the env.d mode, not the per-service template (v4)** — the v4 server scaffold (auth_request `/_auth_jwt` → `subnet-acl-gateway:8770/api/auth/verify`, `@auth_401/@auth_403` dashboard redirects, `location /__basic__/` holding the only `auth_basic`) is **always** injected; the per-service conf is **byte-identical** across modes. `ENABLE_ACL` only controls the env.d one-liner: `true` → `set $auth_mode acl;` (ACL via gateway verify), `false` → `set $auth_mode basic;` (Basic via `/__basic__/`, 0 subrequests). `location = /_set_token` is a plain variable proxy to the gateway exchange, returning `$scheme://$http_host$arg_redirect` so the `/go/` cookie flow lands on the same host port — no JWT in any URL.

...
Input: user_name, service_name, label, volumes, passwd, template paths, env_file?
  ...
  │       │       └─ ENABLE_ACL=true? → JWT+ACL rewrite (auth_request /_auth_jwt, dashboard redirects, no auth_basic)
...
13. **`ENABLE_ACL` switches the nginx auth model** — with `true`, per-service nginx confs use JWT+ACL enforcement (`auth_request /_auth_jwt` to `subnet-acl-gateway:8770/api/auth/verify`, `error_page 401/403` redirects to the `$dashboard_host` dashboard, `auth_basic` stripped so the password can't bypass ACL). With `false`, the legacy `auth_basic` dialog is kept. `location = /_set_token` uses `$scheme://$http_host$arg_redirect` so the `/go/` cookie flow lands on the same host port.

...
# ---------------------------------------------------------------------------
# Stale-conf migration: v3/v4 → v5 (regeneration path)
# ---------------------------------------------------------------------------
# v4 removed the http-level `$is_browser` Accept map from nginx.provision.conf
# (client type is decided by the gateway and relayed via X-Client-Type).
# Per-service confs generated by v3 still reference it, so a fresh nginx build
# crash-loops with `[emerg] unknown "is_browser" variable`. v5 removes the v4
# ACL scaffold (`auth_mode`, `/_auth_jwt`, `/_set_token`, `/__basic__/`,
# @auth_401/@auth_403, env.d) from the internal confs entirely (decisions 6/16).
# `regenerate_nginx_confs` re-renders every registered conf from its template
# (byte-identical v5 simple output) and surgically strips leftover `$is_browser`
# / v4 ACL scaffold from orphan confs that have no registry entry.
...
# Any token that signals a stale v3/v4 ACL scaffold inside an internal conf.
_V4_SCAFFOLD_TOKENS_RE = re.compile(
    r"auth_request|auth_mode|/_auth_jwt|/_set_token|/__basic__/|"
...
def regenerate_nginx_confs() -> dict[str, Any]:
    """Re-render every registered per-service nginx conf with the v5 renderer.

    Migration path for stale deployed confs (v3 ``$is_browser`` refs + v4 ACL
    scaffold). Two passes:

    1. For every registry entry with a template, re-render the conf in place
       via ``template_engine.render_nginx_conf`` — the v5 simple ACL-free form
       (byte-identical across minimal/fullset), no scaffold.
    2. For any conf that still carries stale tokens (orphans with no registry
       entry, or entries whose re-render failed), surgically strip ``$is_browser``
       and the v4 ACL scaffold so nginx can parse the file again.
    ...
    # Pass 2: surgically strip $is_browser / v4 scaffold from remaining confs.
    stripped = 0
    strip_errors: list[str] = []
    for cf in sorted(generated_dir.glob("*.nginx.conf")):
        try:
            content = cf.read_text()
            if "$is_browser" in content or _V4_SCAFFOLD_TOKENS_RE.search(content):
                content = strip_is_browser_refs(content)
                content = template_engine.strip_v4_scaffold(content)
                cf.write_text(content)
                stripped += 1
                _log.info("Stripped stale scaffold from %s", cf.name)
        except Exception as exc:
            strip_errors.append(f"{cf.name}: {exc}")
            _log.warning("Failed to strip stale scaffold from %s: %s", cf.name, exc)
...
def recover_on_startup(
    ...
    (v5 simple ACL-free form) so a fresh nginx build never crash-loops on stale
    ``$is_browser`` references / v4 ACL scaffold.
    ...
    # v3/v4→v5 stale-conf migration: write clean confs BEFORE nginx is reloaded so
    ...
    try:
        ...
        if regen["regenerated"] or regen["is_browser_stripped"]:
            _log.info(
                "nginx recovery: regenerated %d conf(s), stripped stale scaffold in %d",
